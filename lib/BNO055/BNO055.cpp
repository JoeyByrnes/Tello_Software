...
Eigen::VectorXd BNO055::getVector(vector_type_t vector_type)
{
  Eigen::Vector3d xyz;
  uint8_t buffer[6];
  memset (buffer, 0, 6);
 ...
 *           REMAP_CONFIG_P7]
 */
void BNO055::setAxisRemap( bno055_axis_remap_config_t remapcode) {
  bno055_opmode_t modeback = _mode;

  setMode(OPERATION_MODE_CONFIG);
  
  i2c->write_byte(BNO055_AXIS_MAP_CONFIG_ADDR, remapcode);
...
void BNO055::setAxisSign(bno055_axis_remap_sign_t remapsign) {
  ...
  i2c->write_byte(BNO055_AXIS_MAP_SIGN_ADDR, remapsign);
...
void BNO055::setActivePage(uint8_t page){
  bno055_opmode_t modeback = _mode;
  setMode(OPERATION_MODE_CONFIG);

  i2c->write_byte(BNO055_PAGE_ID_ADDR, page);

  setMode(modeback);

}
int  BNO055::getActivePage(){
  return 0;
}

void BNO055::setAccel_16G_1000Hz(){

  bno055_opmode_t modeback = _mode;
  setMode(OPERATION_MODE_CONFIG);

  uint8_t reg_data = 0b00011111;
  i2c->write_byte(BNO055_PAGE_ID_ADDR, 1);
  i2c->write_byte(BNO055::BNO055_ACCEL_CONFIG_ADDR, reg_data);
  i2c->write_byte(BNO055_PAGE_ID_ADDR, 0);
  setMode(modeback);

}

void BNO055::setAccelConfig(accel_config_G G, accel_config_Hz Hz){

  uint8_t g_reg = 0b00000000;
  if(G == BNO055_ACCEL_CONFIG_2G ) g_reg = 0b00000000;
  if(G == BNO055_ACCEL_CONFIG_4G ) g_reg = 0b00000001;
  if(G == BNO055_ACCEL_CONFIG_8G ) g_reg = 0b00000010;
  if(G == BNO055_ACCEL_CONFIG_16G) g_reg = 0b00000011;

  uint8_t hz_reg = 0b00000000;
  if(Hz == BNO055_ACCEL_CONFIG_8Hz   )  hz_reg = 0b00000000;
  if(Hz == BNO055_ACCEL_CONFIG_16Hz  )  hz_reg = 0b00000100;
  if(Hz == BNO055_ACCEL_CONFIG_32Hz  )  hz_reg = 0b00001000;
  if(Hz == BNO055_ACCEL_CONFIG_64Hz  )  hz_reg = 0b00001100;
  if(Hz == BNO055_ACCEL_CONFIG_125Hz )  hz_reg = 0b00010000;
  if(Hz == BNO055_ACCEL_CONFIG_250Hz )  hz_reg = 0b00010100;
  if(Hz == BNO055_ACCEL_CONFIG_500Hz )  hz_reg = 0b00011000;
  if(Hz == BNO055_ACCEL_CONFIG_1000Hz)  hz_reg = 0b00011100;
 
  bno055_opmode_t modeback = _mode;
  setMode(OPERATION_MODE_CONFIG);

  uint8_t reg_data = 0b00000000 | g_reg | hz_reg;
  i2c->write_byte(BNO055_PAGE_ID_ADDR, 1);
  i2c->write_byte(BNO055::BNO055_ACCEL_CONFIG_ADDR, reg_data);
  i2c->write_byte(BNO055_PAGE_ID_ADDR, 0);
  setMode(modeback);
