...
// Define HTM (vertice to CoM)
    ...
    HTMcom2v1 << Eigen::Matrix3d::Identity(), Eigen::Vector3d(p_torso(0)/2, p_torso(1)/2, -p_torso(3)), 0, 0, 0, 1;
    HTMcom2v2 << Eigen::Matrix3d::Identity(), Eigen::Vector3d(p_torso(0)/2, -p_torso(1)/2, -p_torso(3)), 0, 0, 0, 1;
    HTMcom2v3 << Eigen::Matrix3d::Identity(), Eigen::Vector3d(-p_torso(0)/2, p_torso(1)/2, -p_torso(3)), 0, 0, 0, 1;
    HTMcom2v4 << Eigen::Matrix3d::Identity(), Eigen::Vector3d(-p_torso(0)/2, -p_torso(1)/2, -p_torso(3)), 0, 0, 0, 1;
    HTMcom2v5 << Eigen::Matrix3d::Identity(), Eigen::Vector3d(p_torso(0)/2, p_torso(1)/2, p_torso(2) - p_torso(3)), 0, 0, 0, 1;
    HTMcom2v6 << Eigen::Matrix3d::Identity(), Eigen::Vector3d(p_torso(0)/2, -p_torso(1)/2, p_torso(2) - p_torso(3)), 0, 0, 0, 1;
    HTMcom2v7 << Eigen::Matrix3d::Identity(), Eigen::Vector3d(-p_torso(0)/2, p_torso(1)/2, p_torso(2) - p_torso(3)), 0, 0, 0, 1;
    HTMcom2v8 << Eigen::Matrix3d::Identity(), Eigen::Vector3d(-p_torso(0)/2, -p_torso(1)/2, p_torso(2) - p_torso(3)), 0, 0, 0, 1;
...
void dash_kin::SRB_FK(MatrixXd& torso_vertices, MatrixXd& right_leg, MatrixXd& left_leg, MatrixXd& lfv, SRB_Params srb_params, VectorXd x, VectorXd q) // TODO: Not finished
{
    // Parameters
    ...
    Eigen::Vector3d pcom = Eigen::Map<Eigen::Vector3d>(x.data(), 3);
    Eigen::Matrix3d R = Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>>(x.data() + 6, 3, 3);

    // get leg joint angles for each leg
    Eigen::VectorXd qr = q.segment(0, 5);
    Eigen::VectorXd ql = q.segment(5, 5);

    // torso to world
    Eigen::Matrix<double,4,4> HTMwd2com;
    HTMwd2com << R(0,0), R(0,1), R(0,2), pcom(0),
                 R(1,0), R(1,1), R(1,2), pcom(1),
                 R(2,0), R(2,1), R(2,2), pcom(2),
                      0,      0,      0,       1;
    ...
    lfv << right_leg.col(3), right_leg.col(4), left_leg.col(3), left_leg.col(4);
}
...
MatrixXd dash_kin::SRB_IK(SRB_Params srb_params, VectorXd CoM, MatrixXd R, MatrixXd lfv)
{
    ...
    Eigen::Vector3d h2lf1R_b_p, h2lf2R_b_p, h2lf1L_b_p, h2lf2L_b_p;

    // task-space bipedal robot states
    pc_curr = CoM;
    R_curr = R;
    lf1R = lfv.block<3,1>(0,0);
    lf2R = lfv.block<3,1>(3,0);
    lf1L = lfv.block<3,1>(6,0);
    lf2L = lfv.block<3,1>(9,0);
    ...
    hr_wd_p = HTMwd2hr.block<3,1>(0,3);
    hl_wd_p = HTMwd2hl.block<3,1>(0,3);
    ...
    q_leg_R = SRB_Leg_IK(srb_params, h2lf1R_wd_p, h2lf2R_wd_p);
    q_leg_L = SRB_Leg_IK(srb_params, h2lf1L_wd_p, h2lf2R_wd_p);

    q_leg << q_leg_R, q_leg_L;
    return q_leg;
}

void dash_kin::SRB_Kin(VectorXd& q, VectorXd& qd, MatrixXd* Jv_mat, SRB_Params srb_params, VectorXd x, MatrixXd lfv, MatrixXd lfdv)
{
    ...
    double foot_length = srb_params.foot_length; // toe length in m (L3)
    double heel_length = srb_params.heel_length; // heel length in m (L4)
    double num_end_effector_pts = NUM_EE_POINTS; // two per foot
    double position_vec_size = POSITION_VEC_SIZE; // x, y, z components
    ...
    double leg_DoF = LEG_DOF; // leg number of degrees of freedom
    ...
    VectorXd qr = q.segment(0, leg_DoF);
    VectorXd ql = q.segment(leg_DoF, leg_DoF);
    ...
    MatrixXd r_mat = Eigen::Map<MatrixXd>(lfv.data(), position_vec_size, num_end_effector_pts) - pc_curr.replicate(1, num_end_effector_pts);
    // calculate joint velocities
    Vector3d ww_curr = R_curr*wb_curr; // angular velocity in world frame
    MatrixXd qd_b;
    qd_b << pc_curr, ww_curr; // floating base velocities
    MatrixXd lfd_mat = Eigen::Map<MatrixXd>(lfdv.data(), position_vec_size, num_end_effector_pts); // matrix form of end-effector velocities

    
    MatrixXd r_mat_right = r_mat.col(right_end_effector_idx); // right end-effector positions
    MatrixXd lfd_mat_right = lfd_mat.col(right_end_effector_idx); // right end-effector velocities
    ...
    MatrixXd r_mat_left = r_mat.col(left_end_effector_idx); // left end-effector positions
    MatrixXd lfd_mat_left = lfd_mat.col(left_end_effector_idx); // left end-effector velocities
    VectorXd qdl = calc_joint_vel(qd_b, R_curr, r_mat_left, lfd_mat_left, J_mat[left_end_effector_idx]); // left leg joint velocities

    VectorXd qd1(qdr.size() + qdl.size());
    qd1 << qdr, qdl; // joint velocities
    qd = qd1;
...
VectorXd dash_kin::calc_joint_vel(MatrixXd qd_b, Matrix3d Rwb,Vector3d x_torso, MatrixXd x_dot_trans, MatrixXd J_leg)
{
    ...
    Vector3d x_dot_rot = VectorXd::Zero(3);
    VectorXd x_dot;
    x_dot << x_dot_trans, x_dot_rot;

    // Calculate Jacobian from floating base to end effector
    MatrixXd J_b(6, J_leg.cols());
    J_b << Matrix3d::Identity(), -dash_utils::hatMap(x_torso), Matrix3d::Zero(), Matrix3d::Identity();

    // Calculate A
    MatrixXd A(6, J_leg.cols());
    A << Rwb, Matrix3d::Zero(), Matrix3d::Zero(), Rwb;
    A *= J_leg;

    // Calculate joint velocities
    VectorXd qd = A.completeOrthogonalDecomposition().solve(x_dot - J_b*qd_b);
